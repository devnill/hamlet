...
## Summary
Implement `TerrainGenerator` class that produces deterministic terrain from a seed using Perlin noise or seeded random.
...
class TerrainGenerator:
    ...
    def __init__(self, config: TerrainConfig | None = None) -> None:
        ...
        self._seed = self._config.seed if self._config.seed is not None else random.randint(0, 2**31 - 1)
    ...
    def _generate_with_noise(self, position: Position) -> "TerrainType":
        ...
        water_val = noise.pnoise2(x * self._config.water_frequency,
                                  y * self._config.water_frequency,
                                  base=self._seed)
        mountain_val = noise.pnoise2(x * self._config.mountain_frequency,
...
- pyproject.toml: Add `noise` as optional dependency (or keep as fallback-only)

## Acceptance Criteria

- `TerrainGenerator(seed=42).generate_terrain(Position(5, 5))` returns same result every time
